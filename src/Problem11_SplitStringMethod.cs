...
namespace Project_Euler
{
    class Problem
    {
        static void Main(string[] args)
        {
            ...
            i = 0;
            j = 0;
            for (int count = 0; count < strArr.Length + 20; count++)
            {
                
                //Console.WriteLine(strArr[count]);
                str = strArr[count];
                Console.Write(str + " ");
                number = 10 * Convert.ToInt32(Char.GetNumericValue(str, 0));
                number = number + Convert.ToInt32(Char.GetNumericValue(str, 1));
                array[i, j] = number;
                i++;
                if (i == 19)
                {
